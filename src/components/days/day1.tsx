...
export function Day1() {
  ...
  const solve1 = (input: string) => {
    ...
    const differenceSum = left.reduce((sum, leftNum, i) => sum + Math.abs(right[i] - leftNum), 0)
  ...
  const solve2 = (input: string) => {
    ...
    const [left, right] = splitLists(input);
    const result = left.reduce((sum, leftNum, i) => sum + leftNum * right.filter(num => num === leftNum).length, 0)
    return result
  };
